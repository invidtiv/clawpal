        ...
        for candidate in candidates {
            if let Ok(v) = serde_json::from_str::<Value>(&candidate) {
                ...
                if tool == Some("system.run") {
                    ...
                    let note = format!("建议执行诊断命令：`{}`\n原因：{}", payload["args"]["command"].as_str().unwrap_or(""), reason);
...
mod tests {
    ...
    fn parse_tool_intent_handles_mixed_text_with_embedded_json() {
        let raw = r#"好的，我来检查。
{"tool":"system.run","type":"read","command":"ls -la","reason":"查看目录结构"}"#;
    ...
    fn parse_tool_intent_picks_tool_json_when_multiple_json_objects_exist() {
        let raw = r#"前置说明 {"step":"verify","reason":"ignore this"} 然后执行 {"tool":"system.run","type":"read","command":"pwd","reason":"确认路径"}"#;
        let parsed = ZeroclawDoctorAdapter::parse_tool_intent(raw);
        assert!(parsed.is_some(), "should parse tool JSON even if another JSON appears first");
    }
}
