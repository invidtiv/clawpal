...
impl SshConnectionPool {
    ...
    pub async fn exec(&self, id: &str, command: &str) -> Result<SshExecResult, String> {
        let core_cfg = self.lookup_core_config(id).await?;
        let session = clawpal_core::ssh::SshSession::connect(&core_cfg)
            .await
            .map_err(|e| e.to_string())?;
        let result = session.exec(command).await.map_err(|e| e.to_string())?;
        Ok(SshExecResult {
...
case \"$LOGIN_SHELL\" in \
  ...
  *) \"$LOGIN_SHELL\" -lc 'eval \"$CLAWPAL_LOGIN_CMD\"' ;; \
    ...
    async fn lookup_core_config(
        &self,
        id: &str,
    ) -> Result<clawpal_core::instance::SshHostConfig, String> {
        let guard = self.connections.lock().await;
        let conn = guard
            .get(id)
            .ok_or_else(|| format!("No connection for id: {id}"))?;
